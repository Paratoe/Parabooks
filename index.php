  ...
  <?php foreach ($books as $b): ?>
    <article class="card mb-3">
      <div class="card-body">
        ...
        <h2 class="h5 mb-1"><?= htmlspecialchars($b['title']) ?></h2>

        <!-- Geen heading, maar styled tekst -->
        <p class="text-muted mb-2">
          <?= htmlspecialchars($b['author']) ?> — <?= htmlspecialchars($b['genre']) ?>
        </p>
        ...
        <a href="delete_book.php?id=<?= $b['id'] ?>" class="btn btn-outline-danger btn-sm"
           onclick="return confirm('Verwijderen?')">Verwijder</a>
